...
export const cacheHandler = <
    ...
    ReqBody = any,
    ReqQuery extends core.Query = core.Query
>(options: CacheOptions<P, ResBody, ReqBody, ReqQuery> = {}) : RequestHandler<P, ResBody, ReqBody, ReqQuery> => {
    return (req, res, next) => {
        ...
        try {
            ...
            if (cachedResponse) {
                ...
                if (options.public) {
                    res.setHeader('Last-Modified', new Date(cacheService.getTtl(key)! * 1000).toUTCString());
                    res.setHeader('Expires', new Date(cacheService.getTtl(key)! * 1000).toUTCString());
                }
            ...
            // CACHE MISS!
                ...
                res.json = ((data: any) => {
                    ...
                    if (options.public) {
                        ...
                        res.setHeader('Expires', new Date(Date.now() + (options.ttl || 0) * 1000).toUTCString());
                    }
